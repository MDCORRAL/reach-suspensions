...
JS_PATH = PROJECT_ROOT / "dashboard" / "data" / "pareto_grade_setting_payload.js"
...
def compute_slide_rows(frame: pd.DataFrame) -> list[SlideRow]:
    ...
    for (year_num, level, setting), group in grouped:
        ...
        if total_measure <= 0:
            continue
        for pct in TOP_PCTS:
            cutoff = max(1, int(math.ceil(pct * n_schools)))
...
def write_outputs(rows: Iterable[SlideRow]) -> Path:
    ...
    banner = "// Auto-generated by build_pareto_grade_setting_payload.py. Do not edit manually.\n"
    assignment = "window.TAIL_CONCENTRATION_DATA = " + json.dumps(payload, ensure_ascii=False) + ";\n"
    JS_PATH.write_text(banner + assignment, encoding="utf-8")
    return csv_path
...
def main() -> None:
    ...
    print(
        "Updated "
        f"{JSON_PATH.relative_to(PROJECT_ROOT)} and {JS_PATH.relative_to(PROJECT_ROOT)}"
    )
